...
export const analyzeCommits = async (_, context) => {
  ...
  const {parserPreset, rules} = await load();

  if (isEmpty(rules)) {
    logger.warn(
      'Rules`s commitlint configuration is empty.',
      'See the https://commitlint.js.org/reference/configuration.html#configuration for more details.',
    );
  }
